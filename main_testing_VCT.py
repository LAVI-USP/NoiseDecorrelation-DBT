...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 11 14:20:29 2022
...
from libs.utilities import load_model_gan, makedir
...
def model_forward(model, img_ld, red_factor, batch_size, vmin, vmax):
    
    # Change model to eval
    model.eval()

    # Normalize image
    img_ld = scale(img_ld, red_factor=red_factor, vmin=vmin, vmax=vmax)    
    ...
    # Normalize image (Inv)
    rst_img = de_scale(rst_img, vmin=vmin, vmax=vmax)
    
    return rst_img

def test(model, data2read, path2write, red_factor, mAsLowDose, batch_size, nrlz, vmin, vmax):
    
    
    for folder_name in tqdm(data2read):
        
        patient_name = folder_name.split('/')[-1]
        
        for rlz in range(1,nrlz+1):
    
            folder_name_wc = '/'.join(folder_name.split('/')[:-1]) + '/noisy/' + patient_name + '-{}mAs-rlz{}-wc'.format(90,rlz)
    
            file_names = list(pathlib.Path(folder_name_wc).glob('**/*'))
            ...
            for file_name in file_names:
                ...
                rst_img = model_forward(model, img_ld, red_factor, batch_size, vmin, vmax)
        
                folder2write_name = path2write + patient_name + '-{}mAs-rlz{}-uc-{}'.format(mAsLowDose,rlz,target_mAs_arg_fine) 
                file2write_name = file_name.split('/') [-1]
                
                # Create output dir (if needed)
                makedir(folder2write_name)
                
                # Copy the restored data to the original dicom header
                dcmH.PixelData = np.uint16(rst_img)
                
                # Write dicom
                pydicom.dcmwrite(os.path.join(folder2write_name,file2write_name),
                                 dcmH, 
...
if __name__ == '__main__':
    ...
    if mAsFullDose == 360:
        target_mAs_arg = '_360mAs'
        target_mAs_arg_fine = 'to360mAs'
        # 360mAs
        vmin=2000
        vmax=18000
    else:
    ...
    path2write = "data/outputs/"
    
    data2read = ['/media/rodrigo/Data/images/UPenn/Phantom/VCT/VCT_Bruno_500/GE-projs/Jhones_888076.1.555465657049.20200828021423277-proj']
    
    makedir(path2write)
    
    path_final_model = path_models + 'cGAN_ResNet_Decor-90mAs{}.pth'.format(target_mAs_arg)
    ...
    _ = load_model_gan([model, None, None, None],path_final_model=path_final_model)
    
    print("Running test on {}.".format(device))
    
    test(model, data2read, path2write, red_factor, mAsLowDose, batch_size, nrlz, vmin, vmax)
